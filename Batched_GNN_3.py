...
L1_SIZE     = EMBED_SIZE * 4 * 2
L2_SIZE     = EMBED_SIZE * 4 * 2 * 2
L3_SIZE     = EMBED_SIZE * 4 * 2 * 2 * 2
...
class GNNClassifier(torch.nn.Module):
    def __init__(self, num_classes, global_graph):
        ...
        self.position_embedding_1          = torch.nn.Embedding(140, 64) # Maybe can just go directly to 16 with no Relu
        ...
        self.conv1 = GCNConv(L0_SIZE, L1_SIZE)
        self.conv2 = GCNConv(L1_SIZE, L2_SIZE)
        self.conv3 = GCNConv(L2_SIZE, L3_SIZE)
    ...
    def forward(self, climbs):
        ...
        #climb_node_features = climb_node_features.to(torch.float32)
        ...
        x1 = self.conv1(x , climb_edge_indexes, edge_weight = weights)
        x1 = F.leaky_relu(x1, negative_slope=0.01)
        x1 = F.dropout(x1, p = 0.2)
        x2 = self.conv2(x1, climb_edge_indexes, edge_weight = weights)
        x2 = F.leaky_relu(x2, negative_slope=0.01)
        x2 = F.dropout(x2, p = 0.2)
        x3 = self.conv3(x2, climb_edge_indexes, edge_weight = weights)
        x3 = F.leaky_relu(x3, negative_slope=0.01)
        x3 = F.dropout(x3, p = 0.2)

        xF = x3 + self.skip13(x1) + self.skip23(x2)
